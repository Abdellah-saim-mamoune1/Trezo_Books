...
namespace EcommerceBackend.WebAPI.Controllers.PublicControllers
{
    ...
    [ApiController]
    public class AuthorizationController(IAuthService _AuthService) : ControllerBase
    {

        [HttpPost("login")]
        public async Task<IActionResult> Login([FromBody] DLogin request)
        {
        ...
        public async Task<IActionResult> RefreshToken(string Role)
        {
                ...
                var data = await _AuthService.RefreshTokens(new DRefreshTokenRequest
                {
        ...
        public IActionResult DeleteCookies()
        {
            ...
            return Ok(new { message = "Cookies deleted successfully" });
        }

...
namespace EcommerceBackend.Core.Application.Services.AuthenticationServices
{
        ...
        ) : IAuthService
    {
        ...
        public async Task<ApiResponseDto<TokenResponseDto?>> Login(LoginDto form)
        {
            var Errors = await _Validate.ValidateLogin(form);

            if (Errors != null)
            {
                return UApiResponder<TokenResponseDto>.Fail("Invalid pieces of information.", Errors, 400);
            }
            ...
            return UApiResponder<TokenResponseDto>.Success(data, "Login was successful.");
        }


        public async Task<ApiResponseDto<TokenResponseDto?>> RefreshTokens(RefreshTokenRequestDto form)
        {
            ...
            return UApiResponder<TokenResponseDto>.Success(data, "Tokens were refreshed successfully.");
        }

...
namespace EcommerceBackend.Core.Application.Services.AuthenticationServices
{
    public class AuthenticationValidationService(AppDbContext _db): IAuthenticationValidationService
    {
        ...
        public async Task<List<ValidationErorrsDto>?> ValidateLogin(LoginDto request)
        {
            ...
            if (!validate.IsValid)
            {
                errors = validate.Errors.Select(e => new ValidationErorrsDto { FieldId = e.PropertyName, Message = e.ErrorMessage }).ToList();
                return errors;
            }

            else if(!await ValidateEmployeeAccountXPassword(request))
            {
                errors.Add(new ValidationErorrsDto { FieldId = "Account.", Message = "Account was not found." });
            }

              return errors.Count != 0 ? errors : null;
        }

        public async Task<List<ValidationErorrsDto>?> ValidateRefreshToken(RefreshTokenRequestDto request)
        {
            List<ValidationErorrsDto> errors = new();
          
            if (!await ValidateRefreshTokenAsync(request))
            
                errors.Add(new ValidationErorrsDto { FieldId = "DRefreshTokenRequest.RefreshToken.", Message = "Refresh token not found." });
         
            if(request.Role!="Client"&& request.Role !="Admin"&& request.Role !="Seller")
                errors.Add(new ValidationErorrsDto { FieldId = "DRefreshTokenRequest.Role.", Message = "Invalid role." });

            return errors.Count != 0 ? errors : null;
        }
        private async Task<bool> ValidateEmployeeAccountXPassword(LoginDto request)
        {
        ...
        private async Task<bool> ValidateRefreshTokenAsync(RefreshTokenRequestDto request)
        {

...
namespace EcommerceBackend.Core.Application.Services.EmployeeServices.EAuthorServices
{
       ...
       ) : IAuthorManagementValidationService
    {
        public async Task<List<ValidationErorrsDto>?> ValidateAdd(AuthorDto author)
        {
            ...
            {
                errors.Add(new ValidationErorrsDto { FieldId = "Author", Message = "Author already exists." });
            }
        ...
        public async Task<List<ValidationErorrsDto>?> ValidateDelete(int AuthorId)
        {
            List<ValidationErorrsDto> errors = new();
            if (!await AuthorExistsById(AuthorId))
            {
                errors.Add(new ValidationErorrsDto { FieldId = "AuthorId", Message = "No author found." });
                return errors;
            }
            if (await AuthorBooksExists(AuthorId))
            {
                errors.Add(new ValidationErorrsDto { FieldId = "Author books", Message = "Cannot delete author because they have books." });
                return errors;
            }
            return errors.Count != 0 ? errors : null;
        }
        ...
        public async Task<List<ValidationErorrsDto>?> ValidateGetPaginated(PaginationFormDto Form)
        {
            List<ValidationErorrsDto> errors = new();

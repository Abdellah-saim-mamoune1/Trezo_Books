...
{
    public class BookManagementValidationService
        (

       AppDbContext _db

        ) : IBookManagementValidationService
    {
        public async Task<List<ValidationErorrsDto>?> ValidateAdd(AddBookDto searchForm)
        {
            ...
            if (!result.IsValid)
            {
             ...
             if(!await AuthorExistsByName(searchForm.Author))
                errors.Add(new ValidationErorrsDto { FieldId = "Author", Message = "author not found." });
        ...
        public async Task<List<ValidationErorrsDto>?> ValidateUpdate(BookGetXUpdateDto book)
        {
            List<ValidationErorrsDto> errors = new();
            ...
            if (!result.IsValid)
            {
             ...
             if (!await BookTypeExistsById(book.TypeId))
                errors.Add(new ValidationErorrsDto { FieldId = "TypeId", Message = "book type not found." });
             if (!await AuthorExistsById(book.AuthorId))
                errors.Add(new ValidationErorrsDto { FieldId = "AuthorId", Message = "author not found." });
             if (!await BookExistsById(book.Id))
                errors.Add(new ValidationErorrsDto { FieldId = "Id", Message ="Book not found." });
        ...
        public async Task<List<ValidationErorrsDto>?> ValidateDelete(int Id)
        {
             if (!await BookExistsById(Id))
               return new List<ValidationErorrsDto> { new ValidationErorrsDto { FieldId = "Id", Message = "Book not found." }};

             if(await BookCopyExistsById(Id))
                return new List<ValidationErorrsDto> { new ValidationErorrsDto { FieldId = "Id", Message = "Book have copies." } };
            return null;
        }

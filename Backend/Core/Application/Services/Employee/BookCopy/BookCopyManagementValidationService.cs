...
{
    public class BookCopyManagementValidationService
        (
       AppDbContext _db

        ): IBookCopyManagementValidationService
    {


        public async Task<List<ValidationErorrsDto>?> ValidateAdd(BookCopyDto bookCopy)
        {
            ...
            if (!await BookExistsById(bookCopy.BookId))
                errors.Add(new ValidationErorrsDto { FieldId = "BookId", Message = "Book not found." });
            if(await ValidateBookAlreadyHasCopyByIdAsync(bookCopy.BookId))
                errors.Add(new ValidationErorrsDto { FieldId = "BookId", Message = "Book already has a copy." });

            return errors.Count == 0 ? null : errors;
        }


        public async Task<List<ValidationErorrsDto>?> ValidateUpdate(DBookCopyGetXUpdate bookCopy)
        {
            List<ValidationErorrsDto> errors = new();
            var validator = new BookCopyValidator();

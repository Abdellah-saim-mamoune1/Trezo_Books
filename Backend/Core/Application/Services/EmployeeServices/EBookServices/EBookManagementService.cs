...
{
    public class EBookManagementService
        (
        
        AppDbContext _db,
         IEBookRepository _Repo,
         IEBookManagementValidationService _Validate
        ): IEBookManagementService
    {

        public async Task<DApiResponse<object?>> CreateBookAsync(string ISBN)
        {
            var Errors = new List<DValidationErorrs>();
            ...
            if (!response.IsSuccessStatusCode)
            {
                return UApiResponder<object>.Fail("Internal server error",
                    new List<DValidationErorrs> { new DValidationErorrs { FieldId = "GoogleAPI", Message = "GoogleAPI error." } }, 500);
            }
            ...
            catch (JsonException)
            {
                return UApiResponder<object>.Fail("Invalid JSON response from Google Books",
                    new List<DValidationErorrs> { new DValidationErorrs { FieldId = "GoogleAPI", Message = "Invalid JSON structure." } }, 500);
            }
            ...
            if (!booksData.RootElement.TryGetProperty("items", out var items) || items.ValueKind != JsonValueKind.Array || !items.EnumerateArray().Any())
            {
                return UApiResponder<object>.Fail("Book not found",
                    new List<DValidationErorrs> { new DValidationErorrs { FieldId = "ISBN", Message = "Book not found on Google Books." } }, 404);
            }
            ...
            if (!firstBook.TryGetProperty("volumeInfo", out var volumeInfo))
            {
                return UApiResponder<object>.Fail("Bad request",
                    new List<DValidationErorrs> { new DValidationErorrs { FieldId = "ISBN", Message = "Missing volumeInfo in book data." } }, 400);
            }
            ...
            {
                Errors.Add(new DValidationErorrs { FieldId = "Title", Message = "Book title not found." });
            }
            ...
            if (TypeId == -1)
            {
                Errors.Add(new DValidationErorrs { FieldId = "Category", Message = "Book category not found in system." });
            }
            ...
            {
                return UApiResponder<object>.Fail("Internal server error",
                    new List<DValidationErorrs> { new DValidationErorrs { FieldId = "Server", Message = "Failed to insert book into database." } }, 500);
            }
        ...
        public async Task<DApiResponse<object?>> UpdateBookAsync(DEBookGetXUpdate book)
        {
        ...
        public async Task<DApiResponse<object?>> DeleteBookAsync(int BookId)
        {
        ...
        public async Task<DApiResponse<object?>> GetPaginatedBooksAsync(DPaginationForm Form)
        {
        ...
        public async Task<DApiResponse<object?>> GetBookByIdAsync(int Id)
        {
        ...
        public async Task<DApiResponse<object?>> GetBookByNameAsync(string Name)
        {

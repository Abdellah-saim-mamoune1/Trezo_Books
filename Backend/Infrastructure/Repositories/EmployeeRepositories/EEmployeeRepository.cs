...
{
    public class EEmployeeRepository(AppDbContext _db):IEEmployeeRepository
    {
        public async Task<DTokenResponse?> RegisterAsync(DEEmployeeSignUp SignUpInfos)
        {
            ...
            string password = new PasswordHasher<DEEmployeeSignUp>()
        ...
        public async Task<bool> UpdateAsync(DPerson form,int Id)
        {
            try
            {
                await _db.Database.CreateExecutionStrategy().ExecuteAsync(async () =>
        ...
        public async Task<DEGetEmployees> GetByIdAsync(int Id)
        {
            var data = await _db.Employees.Include(e => e.Person).Include(e => e.EmployeeAccount).ThenInclude(e => e!.EmployeeAccountType)
                .AsQueryable().Where(e=>e.PersonId==Id).Select(e => new DEGetEmployees
                {
                    Id=e.PersonId,
                    Address = e.Person!.Address!,
                    BirthDate = e.Person.BirthDate,
        ...
        private async Task<int> RegisterPerson(DEEmployeeSignUp SignUpInfos)
        {

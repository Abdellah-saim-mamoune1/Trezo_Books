...
{
    public class ClientRepository(AppDbContext _db): IClientManagementRepository
    {

        public async Task<TokenResponseDto?> RegisterClientAsync(ClientSignUpDto SignUpInfos)
        {
            var Tokens=new TokenResponseDto();
            string password = new PasswordHasher<ClientSignUpDto>()
        ...
        public async Task<bool> UpdateProfileInfoAsync(UpdateClientProfileInfoDto Form, int Id)
        {
            bool Success = false;
            try
            {
        ...
        public async Task<GetClientInfoDto?> GetClientInfoByIdAsync(int ClientId)
        {
            try
            {

                return await _db.Clients.Include(c => c.Person).Include(c => c.Account)
                    .AsQueryable().Where(c => c.PersonId == ClientId).Select(c => new GetClientInfoDto
                    {
        ...
        private async Task<int> SignUpPerson(ClientSignUpDto SignUpInfos)
        {

...
using EcommerceBackend.Infrastructure.db_samples;
...
using (var scope = app.Services.CreateScope())
{
    var Seeder = scope.ServiceProvider.GetRequiredService<SampleDataSeederService>();
   

    var sampleFile = Path.Combine(AppContext.BaseDirectory,"Infrastructure", "db_samples", "sample_data.json");
    if (File.Exists(sampleFile))
    {
        var json = await File.ReadAllTextAsync(sampleFile);
